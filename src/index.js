...
function moveRect(event) {
  ...
  while (newTop == rectTop && newLeft == rectLeft) {
    if (newTop == rectTop) newTop = Math.floor(Math.random() * browserH);
    else newLeft = Math.floor(Math.random() * browserW);
  }

  var correctL = false,
    correctT = false;
  while (!correctL || !correctT) {
    //it wont go out of browser viewport
    if (newLeft + rectWidth <= browserW) correctL = true;
    else newLeft = Math.floor(Math.random() * browserW);

    if (newTop + rectHeight <= browserH) correctT = true;
    else newTop = Math.floor(Math.random() * browserH);
  }
...
function sendToDesti(finalTop, finalLeft, id) {
  ...
  if (intialLeft < finalLeft) {
    if (intialLeft + 20 <= finalLeft) intialLeft += 20;
    else intialLeft = finalLeft;
  } else {
    if (intialLeft - 20 >= finalLeft) intialLeft -= 20;
    else intialLeft = finalLeft;
  }
  ...
  if (intialTop < finalTop) {
    if (intialTop + 20 <= finalTop) intialTop += 20;
    else intialTop = finalTop;
  } else {
    if (intialTop - 20 >= finalTop) intialTop -= 20;
    else intialTop = finalTop;
  }
